...
export function DifficultyPicker({
  ...
  return (
    <div className="surface-elevated rounded-3xl p-6">
      <h3 className="text-center text-sm uppercase tracking-widest text-white/40 mb-1">
        ...
        {DIFFS.map((d) => {
          ...
          return (
            <motion.button
              ...
              onClick={() => onPick(d)}
              disabled={disabled}
              className="flex flex-col items-center gap-2 px-4 py-4 rounded-2xl border-2 border-bg-border bg-bg-card hover:border-gold/60 hover:bg-gold/5 disabled:opacity-50 transition"
            >
              <Icon className="w-5 h-5 text-gold" />
              <span className="font-bold text-white">Niveau {d}</span>
              <span className="text-xs text-white/40">{labels[d]}</span>
